...
- Various deployments (`host/networking/avahi-cname`, `host/cockpit`, `host/sshd`, `infra/caddy-ingress`, and `infra/mosquitto`) now provide firewalld rules (as drop-in snippet files assembled by deployment `host/networking/firewalld`) to explicitly allow access through the network firewall, both from directly-connected devices (via respective feature flags named `firewall-allow-direct`) in the `nm-shared` firewalld zone and from public LANs (via respective feature flags named `firewall-allow-public`) in the `public` firewalld zone.
...
- (Breaking change) Deployment `host/networking/avahi-daemon` has been renamed to `host/networking/avahi-cname` to reflect its updated functionality.
...
- Deployment `host/networking/cname-daemon`'s systemd services have been renamed from `planktoscope-mdns-alias@pkscope.service` and `planktoscope-mdns-alias@planktoscope.service` to `avahi-publish-cname@pkscope.local.service` and `avahi-publish-cname@planktoscope.local.service`.
