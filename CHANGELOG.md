...
- New deployment `host/networking/networkmanager` adds NetworkManager connection profiles for `eth0`, `eth1`, and `usb0` with internet connection sharing and PlanktoScope-specific IP addresses (`192.168.5.1`, etc.). It also replaces deployment `host/networking/dhcpcd` (see removals below)
...
- New deployment `host/networking/avahi-cname` takes over the responsibility (previously provided by deployment `host/networking/avahi-daemon`) of registering `planktoscope.local` and `pkscope.local` as mDNS names.
- Various deployments (`host/networking/avahi-daemon`, `host/cockpit`, `host/sshd`, `infra/caddy-ingress`, and `infra/mosquitto`) now provide firewalld rules (as drop-in snippet files assembled by deployment `host/networking/firewalld`) to explicitly allow access through the network firewall, both from directly-connected devices (via respective feature flags named `firewall-allow-direct`) in the `nm-shared` firewalld zone and from public LANs (via respective feature flags named `firewall-allow-public`) in the `public` firewalld zone.
...
- New pallet feature flags `networking/firewall` and `base/firewall` (which are identical to each other) are provided for easy integration with the new drop-in firewalld zone generation system.

### Changed

- (Breaking change) Package `core/host/networking/interface-forwarding` has moved its remaining default functionality (see removals described below) into a new feature flag named `planktoscope-inbound` (for forwarding network traffic targeted at `192.168.4.1`, `192.168.5.1`, etc., all to `127.0.0.1`), so that functionality is no longer enabled by default in deployments (though the `host/networking/interface-forwarding` deployment does enable it, resulting in no visible change to the deployment when deployed on a NetworkManager-based system). Similarly, the `/usr/lib/enable-interface-forwarding` script has been renamed to `/usr/libexec/forward-inbound`.
- (Breaking change) Deployment `host/networking/dnsmasq` has been renamed to `host/networking/networkmanager-dnsmasq` (along with its corresponding package) to reflect its updated functionality of configuring the dnsmasq instance launched by NetworkManager on shared network connections.
...
- (Breaking change) Package `core/host/networking/avahi-daemon`'s `register-planktoscope-local` and `register-pkscope-local` feature flags have been moved into a new `core/host/networking/avahi-cname` package.
- (Breaking change) The `dhcp-default-route` feature flag in package `core/host/networking/interface-forwarding` has been moved to package `core/host/networking/networkmanager-dnsmasq` (for eth0/eth1) and package `core/host/networking/networkmanager-hotspot` (for wlan0/wlan1), and it no longer advertises the PlanktoScope as a default to the internet on the `usb0` interface (since that is presumably being used for the attached USB device to share its internet access to the PlanktoScope). This means that the DHCP default route behavior is now toggled independently for ethernet vs. Wi-Fi.
...
- Deployments `apps/ps/backend/proc-segmenter`, `host/cockpit`, `host/machine-name`, `host/networking/hosts`, `host/networking/networkmanager`, `host/networking/networkmanager-dnsmasq`, and `host/networking/networkmanager-hotspot` now load the machine name from `/etc/machine-name` instead of `/run/machine-name` so that the  machine name can be manually overridden via `/etc/machine-name` for template string interpolation.
...
- Deployment `host/networking/autohotspot` (along with its associated package `core/host/networking/autohotspot`) has been replaced with deployment `host/networking/networkmanager-hotspot`.
...
- Deployment `host/networking/avahi-daemon`'s published aliases (`planktoscope.local` and `pkscope.local`, now provided instead by deployment `host/networking/avahi-cname`) should now work regardless of the IP addresses the PlanktoScope is connected on, instead of only pointing to `192.168.4.1` regardless of how the PlanktoScope is being accessed.
