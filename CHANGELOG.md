...
- New deployment `host/config-templating` provides a script `/usr/libexec/render-templated-config` which can be used by other deployments (and now is used by other deployments) for rendering config templates or config template snippets with string interpolation of various variables loaded from `/etc` files (machine name, hostname, and custom DNS domain).
